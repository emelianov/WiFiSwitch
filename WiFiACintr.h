...
uint32_t powerCalc() {
  ...
  // Some magic: skip first 1/3 of samples to use flat data part afer filter
  for (uint16_t i = MAX_SAMPLES / 3; i < MAX_SAMPLES; i++) {
    sampleI = sI[i];
    sampleV = sV[i];
    sqV = sampleV * sampleV;
    sumV += sqV;
  ...
  // MAX_SAMPLES / 1.5 because of 1/3 frucation
  Vrms[mcpCalc] = V_RATIO * sqrt(sumV / (MAX_SAMPLES / 1.5));
  ...
  // MAX_SAMPLES / 1.5 because of 1/3 frucation
  Irms[mcpCalc] = I_RATIO * sqrt(sumI / (MAX_SAMPLES / 1.5));
