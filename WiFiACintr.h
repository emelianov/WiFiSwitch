...
uint32_t pri() {
  ...
  Serial.printf("Real: %s, Apparent: %s, PF: %s, Vrms: %s, Irms : %s, C: %ld\n", String(realPower[0]).c_str(), String(apparentPower[0]).c_str(), String(powerFactor[0]).c_str(), String(Vrms[0]).c_str(), String(Irms[0]).c_str(), c);
 #endif
  return 5000;
}
