function initArduinoIO(){
	...
	$(docWave).change(function(){changeStateWave=!0});
	GetArduinoIO()
}
...
//console.log(docSockets[i].checked?"SOC"+i+"=1&":"SOC"+i+"=0&"),
		...
		changeStateWave&&changeWaveState=!1,n+docWave.value+"&"
	}
	console.log(n+strWaveSelection),
...
function CheckBoxUpdate(x) {
	var e,
	t= Math.min(x.length, docCandles.length);
//	for(e=0;e<t;e++)
//		docCandles[e].value=x[e].childNodes[0].nodeValue!=2?x[e].childNodes[0].nodeValue:""	
}
function GroupsUpdate(x) {
	var e,
	t= Math.min(x.length, docGroups.length);
	for(e=0;e<t;e++)
		docGroups[e].value=x[e].childNodes[0].nodeValue	
}
