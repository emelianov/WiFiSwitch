...
function GetArduinoIO(){
	...
	t.onreadystatechange=
		function(){
					4==this.readyState&&200==this.status&&null!=this.responseXML&&(AnalogXMLupdate(this.responseXML.getElementsByTagName("analog")),
...
//console.log(docSockets[i].checked),
//console.log(docSockets[i].checked?"SOC"+i+"=1&":"SOC"+i+"=0&"),
		...
		for(i=0;i<docTimer.length;i++)
			changeStateTimer[i]&&(changeStateTimer[i]=!1,n+="TIM"+i+"="+$(docTimer[i]).find("input")+"&")
	}
	console.log(n),
	console.log(strWaveSelection),
	t.open("GET","/ajax_inputs?"+n+strWaveSelection+e,!0),
	t.send(null),
...
function SocketXMLupdate(x){
	...
	for(t=0;t<e&&t<docSockets.length;t++)
		...
		console.log("Sockets received: "+i)
}
function TimerToggleXMLupdate(x){
	...
	for(t=0;t<e&&t<docTimerCheckbox.length;t++)
		"checked"===x[t].childNodes[0].nodeValue?(docTimerCheckbox[t].checked||($(docTimerCheckbox[t]).prop("checked",!0),$(docTimerCheckbox[t]).bootstrapToggle("on")),i+="1"):(docTimerCheckbox[t].checked&&($(docTimerCheckbox[t]).prop("checked",!1),
		$(docTimerCheckbox[t]).bootstrapToggle("off")),i+="0");
		console.log("TimerCB received: "+i)
}
function TimerValueXMLupdate(x){
	var e,
	t= x.length;
	for(e=0;e<t&&e<docTimer.length;e++)
		//docTimer[e].innerHTML=x[e].childNodes[0].nodeValue;
		docTimer[e].value=x[e].childNodes[0].nodeValue;
		console.log("TimerCB received: ??")
}
